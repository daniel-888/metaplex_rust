...
const MAX_WHILTELIST_WALLET_LIMIT = 100;
const MAX_WHILTELIST_WALLET_LEN = 32 + 4 + 4;
...
pub mod nft_candy_machine_v2 {
    ...
    pub fn mint_nft<'info>(
        ...
        let wallet_pda = &mut ctx.accounts.wallet_pda; // @Guardian
        ...
        if let Some(ws) = &candy_machine.data.whitelist_mint_settings {
            ...
            match candy_machine.data.go_live_date {
                ...
                Some(val) => {
                        ...
                        && !ws.presale
                    {
                        ...
                        if ws.mode == WalletWhilteList {
                            for whiltelist_item in &mut candy_machine.data.whitelist_wallets {
                                if whiltelist_item.address == payer.key() {
                                    if (whiltelist_item.current_mint_count == whiltelist_item.max_mint_count) {
                                        return Err(ErrorCode::PresaleMintExceeded.into());
                                    }
                                    else {
                                        whiltelist_item.current_mint_count += 1;
                                    }
                                }
                            }
                        }
            ...
            assert_valid_go_live(payer, clock, candy_machine)?;
            // @Guardian
            if wallet_pda.mint_count == max_public_mint_count {
                return Err(ErrorCode::PublicMintExceeded.into());
            }
            else {
                wallet_pda.mint_count += 1;
            }
        }
    ...
    pub fn initialize_candy_machine(
        ...
        // @Guardian
        if candy_machine.data.whitelist_wallets.len() > MAX_WHILTELIST_WALLET_LIMIT - 1 {
            return Err(ErrorCode::TooManyWhiltelistWallets.into());
        }
...
pub struct MintNFT<'info> {
    // @Guardian
    #[account(mut)]
    wallet_pda: Account<'info, WalletPda>,
    #[account(
        mut,
        has_one = wallet
    )]
...
pub struct CandyMachineData {
    ...
    // walletwhitelists
    pub whitelist_wallets: Option<Vec<WhitelistWallet>> // @Guardian
    pub max_public_mint_count: u32 // @Guardian
}
...
1 + 32 + 1  + // gatekeeper
MAX_WHILTELIST_WALLET_LIMIT * MAX_WHILTELIST_WALLET_LEN /// @Guardian whitelist wallet
;
...
//@Guardian WhiltelistWallet struct
#[derive(AnchorSerialize, AnchorDeserialize, Clone)]
...
pub enum ErrorCode {
    ...
    #[msg("Too many whiltelist wallets")] // @Guardian
    TooManyWhiltelistWallets,
    #[msg("Presale mint exceeded")] // @Guardian
    PresaleMintExceeded,
    #[msg("Public mint exceeded")] // @Guardian
    PublicMintExceeded
}
